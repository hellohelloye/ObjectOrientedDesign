...
namespace GraphLib 
{
  ...
  void Vertex<V,E>::add(Edge& edge) 
  {
	  ...
	  {
		  if(_edges[i].first == edge.first
			  && _edges[i].second == edge.second)
			  return;
	  }
  ...
  class Utility
  {
	...
	static void process(int id,graph& g,int& i,std::unordered_map<int,int>& num,
		std::unordered_map<int,int>& lowlink,std::deque<int>& s )
	{
      i = i + 1;
	  num[id] = i;
	  lowlink[id] = i;
	  s.push_back(id);
	  size_t index = g.findVertexIndexById(id);
	  vertex v = g[index];
	  for(size_t k=0;k<v.size();k++)
	  {
	    std::pair<int,E> edge = v[k];
		if(num[edge.first] == 0 )
		{
		   process(edge.first,g,i,num,lowlink,s);
		   lowlink[id] = min(lowlink[id],lowlink[edge.first]);
		}
		else if(num[edge.first]<num[id])
		{
		   if(std::find(s.begin(),s.end(),edge.first)!=s.end())
		   {
			 lowlink[id] = min(lowlink[id],num[edge.first]);
		   }
		}
	  }
	  if(lowlink[id] == num[id])
	  {
		 std::cout<<"\nStrongly connected \n";
		 while(!s.empty())
		 {
			...
			if(num[t]>=num[id])
			{
			   std::cout<<g[g.findVertexIndexById(t)].value()<<" ";
			   s.pop_back();
			}
			else 
			   break;
  ...
  public:
	static void displayStrongConnectedComponents( graph& g)
	{
	  ...
	  {
		num[g[index].id()]=0; 
		lowlink[g[index].id()]=0; 
	  }
	  for(size_t index=0;index<g.size();index++)
	  {
		 if(num[g[index].id()] == 0)
		 {
			//std::cout<<"processing "<<g[index].id()<<std::endl;
			process(g[index].id(),g,i,num,lowlink,s);
		 }
	  }
	}
  };
