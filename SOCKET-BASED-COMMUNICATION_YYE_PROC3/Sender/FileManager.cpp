...
void FileManager::getAllFile(const std::string& path)
{
	...
	std::string newPath;
	std::vector<std::string> currFile = FileSystem::Directory::getFiles(path,"*.h");
	for(size_t i = 0;  i< currFile.size();i++)
	{
		std::cout<<"\n"<<currFile[i].c_str();
		fileList.push_back(currFile[i]);
	}
	std::vector<std::string> currFile2 = FileSystem::Directory::getFiles(path,"*.cpp");
	for(size_t i = 0;  i< currFile.size();i++)
	{
		std::cout<<"\n"<<currFile[i].c_str();
		fileList.push_back(currFile2[i]);
	}
	std::vector<std::string> currFile3 = FileSystem::Directory::getFiles(path,"*.txt");
	for(size_t i = 0;  i< currFile.size();i++)
	{
		std::cout<<"\n"<<currFile[i].c_str();
		fileList.push_back(currFile3[i]);
	}
